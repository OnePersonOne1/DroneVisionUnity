"""COCO class id → name 매핑 (RF-DETR COCO pretrained 헤드와 동일).

RF-DETR (DETR 계열) 은 91-class **sparse** 포맷을 출력한다 — id 는 1..90 이고
12/26/29/30/45/66/68/69/71/83 은 비어 있다. 0-indexed dense (80-class) 와
혼동하면 라벨이 한 칸씩 밀린다 (예: car→motorcycle).

라이브러리 정의 (`rfdetr.assets.coco_classes.COCO_CLASSES`) 를 직접 import 해
영구적으로 일치 보장. rfdetr 미설치 환경 (예: pytorch 만 깔린 워커) 에서는
아래 명시 fallback 사용 — 두 정의는 동일하다.
"""
try:
    from rfdetr.assets.coco_classes import COCO_CLASSES  # type: ignore[import]
except ImportError:
    # rfdetr 미설치 환경 fallback. 라이브러리 정의와 1:1 동일.
    COCO_CLASSES = {
